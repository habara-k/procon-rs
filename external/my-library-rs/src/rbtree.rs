use crate::rbtree_traits::{
    BuildFromSeq, Insert, LazyEval, Merge, Node, RangeFold, Remove, Reversible, Root, Split, Value,
};
...
/// }
///
/// let mut seg: ReversibleRBLazySegtree<RangeAddRangeMin> = vec![1, 100, 1000, 0, 10000].into();
/// assert_eq!(seg.remove(3), 0);  // [1, 100, 1000, 10000]
/// seg.insert(1, 10);  // [1, 10, 100, 1000, 10000]
///
/// assert_eq!(seg.prod(0, 5), 1);
...
/// assert_eq!(seg.get(4), 10020);
///
/// seg.reverse_range(1, 4);  // [3001, 1020, 3120, 3010, 10020];
/// dbg!(seg.get(0));
/// dbg!(seg.get(1));
/// dbg!(seg.get(2));
/// dbg!(seg.get(3));
/// dbg!(seg.get(4));
/// assert_eq!(seg.get(0), 3001);
/// assert_eq!(seg.get(1), 1020);
/// assert_eq!(seg.get(2), 3120);
/// assert_eq!(seg.get(3), 3010);
/// assert_eq!(seg.get(4), 10020);
/// ```
...
impl<F: MapMonoid> Node for ReversibleMapMonoidNode<F> {
    fn l(&self) -> &Option<Box<Self>> {
        &self.l
    }
    fn r(&self) -> &Option<Box<Self>> {
...
impl<F: MapMonoid> Value<<F::M as Monoid>::S> for ReversibleMapMonoidNode<F> {
    fn new_leaf(val: <F::M as Monoid>::S) -> Box<Self> {
        Box::new(Self {
            l: None,
            r: None,
...
impl<F: MapMonoid> Merge for ReversibleMapMonoidNode<F> {}
impl<F: MapMonoid> Split for ReversibleMapMonoidNode<F> {}
impl<F: MapMonoid> Insert<<F::M as Monoid>::S> for ReversibleMapMonoidNode<F> {}
impl<F: MapMonoid> Remove<<F::M as Monoid>::S> for ReversibleMapMonoidNode<F> {}
impl<F: MapMonoid> BuildFromSeq<<F::M as Monoid>::S> for ReversibleMapMonoidNode<F> {}

pub struct ReversibleRBLazySegtree<F: MapMonoid> {
    root: Option<Box<ReversibleMapMonoidNode<F>>>,
}
impl<F: MapMonoid> Root<<F::M as Monoid>::S> for ReversibleRBLazySegtree<F> {
    type Node = ReversibleMapMonoidNode<F>;
    fn root(&self) -> &Option<Box<Self::Node>> {
        &self.root
    }
    fn mut_root(&mut self) -> &mut Option<Box<Self::Node>> {
...
impl<F: MapMonoid> From<Vec<<F::M as Monoid>::S>> for ReversibleRBLazySegtree<F> {
    fn from(v: Vec<<F::M as Monoid>::S>) -> Self {
        Self {
            root: <Self as Root<<F::M as Monoid>::S>>::Node::build(&v, 0, v.len()),
        }
...
impl<F: MapMonoid> Default for ReversibleRBLazySegtree<F> {
    fn default() -> Self {
        vec![].into()
    }
}
